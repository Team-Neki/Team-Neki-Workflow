...
부르는 이유는 좌표가 빈 채로 다음 단계까지 기다리지 않게 하려는 것이고, enrich 는
여기서 못 채운 지점만 같은 locate 로 다시 시도한다. 예외를 유지하는 조건은
docs/spec/collect-pipeline.md 에 있다.
...
- 주소는 해석하지 않는다. 원문을 그대로 질의에 넣을 뿐 층수나 상호명을 떼지
  않는다. enrich 도 떼지 않는다. 법정동 코드는 좌표에서 나오므로 주소를 손질할
  이유가 없다
"""
