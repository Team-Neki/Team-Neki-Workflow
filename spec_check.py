...
정책은 문서가 기준이다. 코드와 어긋나면 코드가 틀린 것이다. 이 검사는 그
우선순위를 건드리지 않고, 항목마다 붙은 `` `경로:줄` `심볼` `` anchor 가 실제
코드 위치를 가리키는지만 본다. 줄이 밀리면 여기서 걸리므로 anchor 의 줄 번호를
고치면 되고, 심볼이 사라졌다면 정책이 바뀐 것이니 문서 본문을 먼저 고친다.
