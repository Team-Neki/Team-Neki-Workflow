...
사용자가 고르는 단위가 그대로 키입니다. "강남"을 검색해 `강남 2호선`과
`강남 신분당선` 중 하나를 고르므로, 그 둘이 한 행을 가리킵니다. 1,098행 전부
유일합니다.

**연번 대리키를 쓰지 않습니다.** 매 실행이 테이블을 새로 만들어 바꿔치므로 연번은
COPY 순서로 매겨지는데, 원문 앞쪽에 역이 하나만 생겨도 그 뒤 전부가 한 칸씩
밀립니다. 실측으로 1,097행 중 1,097행이 어긋나는 것을 확인했습니다. 앱이 들고 있던
id 가 조용히 다른 역을 가리키게 되고, 터지지 않아서 더 나쁩니다.

**원문 식별자(역번호, 노선번호)는 아예 담지 않습니다.** 원문 스스로가 지키지 못해
4건이 부딪힙니다.
...
이것으로 키를 잡으면 합치는 수밖에 없고, 그러면 `광운대 경춘선`과 `총신대입구`가
검색에서 사라집니다. 키가 아닌 데다 검색에도 반경에도 쓰이지 않으므로 남길 이유가
없습니다.

`normalize` 가 합치는 것은 **모든 열이 같은 한 행**(경인선 주안역)뿐입니다.

다음 스냅샷에서 `(name, line_name)` 충돌이 생기면 PK 위반으로 트랜잭션이 통째로
롤백되어 기존 테이블이 살아남습니다. 조용히 틀리는 것보다 낫습니다.

**PK 인덱스로 접두 검색이 되지 않습니다.** 기본 연산자 클래스라 `LIKE '강남%'` 가
타지 않으므로 `name` 에 `text_pattern_ops` 인덱스를 따로 둡니다.
...
환승역구분, 데이터기준일자, 역번호, 노선번호를 담지 않습니다.** 지점 수집에서 픽닷이 지번 주소를
...
```text
name  line_name  location
```

환승 여부는 컬럼이 아니라 같은 `name`의 행 수로 드러납니다. 적재 결과는 1,098행입니다. 시도를 담지 않으므로
