...
#### 키는 `(line_no, station_no)` 입니다

연번 대리키를 쓰지 않습니다. 매 실행이 테이블을 새로 만들어 바꿔치므로 연번은 COPY
순서로 매겨지는데, **원문 앞쪽에 역이 하나만 생겨도 그 뒤 전부가 한 칸씩 밀립니다.**
실측으로 1,097행 중 1,097행이 어긋나는 것을 확인했습니다. 앱이 들고 있던 id 가
조용히 다른 역을 가리키게 되고, 터지지 않아서 더 나쁩니다.

원문이 준 식별자를 키로 쓰면 스냅샷을 새로 받아도 같은 역이 같은 키를 갖습니다.
BACKEND-103 의 "이름은 개편으로 움직이지만 코드로 조인하면 안전하다"와 같은 기준입니다.

**원문이 그 키를 온전히 지키지는 않습니다.** `(line_no, station_no)` 가 겹치는 행이
4건 있어 `normalize` 가 합칩니다.

```text
(I4108, 1019)  광운대 경의중앙선 / 광운대 경춘선     <- 한 노선번호에 두 노선명
(I4108, 1201)  중랑   경의중앙선 / 중랑   경춘선
(I4108, 1202)  상봉   경의중앙선 / 상봉   경춘선
(S1107, 0736)  이수 7호선 / 총신대입구(이수) 7호선   <- 좌표까지 같은데 이름만 둘
```

합쳐지는 승강장 사이가 최대 167m 라 1km 반경에서는 같은 자리로 봐도 됩니다. 대신
**검색에서 사라지는 이름이 생깁니다.** `광운대 경춘선` 과 `총신대입구` 로는 찾을 수
없게 되므로 합친 내역을 로그에 전부 남깁니다.

누구를 남길지는 **같은 스냅샷 안의 다른 행을 근거로** 정합니다. 그 노선번호에서 가장
많이 쓰인 노선명을 가진 행이 남고, 그것도 갈리면 이름 순입니다. 임의로 첫 행을
집으면 원문 순서가 바뀔 때 남는 행이 조용히 달라집니다.

다음 스냅샷에서 합치지 못하는 충돌이 생기면 PK 위반으로 트랜잭션이 통째로 롤백되어
기존 테이블이 살아남습니다. 조용히 틀리는 것보다 낫습니다.
...
```text
line_no  station_no  name  line_name  location
```

환승 여부는 컬럼이 아니라 같은 `name`의 행 수로 드러납니다. 적재 결과는 1,094행입니다. 시도를 담지 않으므로
